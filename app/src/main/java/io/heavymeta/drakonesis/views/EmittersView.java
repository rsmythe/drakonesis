package io.heavymeta.drakonesis.views;
 ...
 */
public class EmittersView extends SurfaceView {
    ...
    public EmittersView(Context context) {
        super(context);
        setupPaints();
    }

    public EmittersView(Context context, AttributeSet attrs) {
        super(context, attrs);
        setupPaints();
    }

    public EmittersView(Context context, AttributeSet attrs, int defStyle) {
        ...
        switch (event.getAction()) {
            case MotionEvent.ACTION_MOVE:
                for (int p = 0; p < event.getPointerCount(); p++) {
                    touchMove(event.getX(p), event.getY(p));
                }
                break;
